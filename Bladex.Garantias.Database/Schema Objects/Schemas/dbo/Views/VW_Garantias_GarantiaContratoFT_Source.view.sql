CREATE VIEW VW_Garantias_GarantiaContratoFT_Source
AS
SELECT     CONVERT(datetime, FLOOR(CONVERT(float,
                      ...
                      dbo.GarantiaBase.InternalStatus, dbo.GarantiaBase.CategoriaSuperId, dbo.GarantiaBase.Source, dbo.GarantiaBase.IndAtomo, 
                      dbo.GarantiaPrenda.ID AS GarantiaPrendaId, dbo.GarantiaPrenda.Emisor AS GarantiaPrendaEmisor, 
                      dbo.GarantiaPrenda.TipoInstrumentoFinanciero AS GarantiaPrendaTipoInstrumentoFinanciero, 
                      ...
                      dbo.GarantiaContrato.DealReference AS LineaCreditoRef, dbo.GarantiaContrato.NetBalancePrincipal, 
                      CASE WHEN isnull(dbo.GarantiaBase.getValorGarantiaSuperIntendencia, 0) - isnull(dbo.GarantiaBase.ValorNecesarioDeGarantia, 0) < 0 THEN (- 1.0) 
                      * (isnull(dbo.GarantiaBase.getValorGarantiaSuperIntendencia, 0) - isnull(dbo.GarantiaBase.ValorNecesarioDeGarantia, 0)) ELSE 0 END AS ValorInsuficienciaGarantia, 
                      dbo.GarantiaContrato.ID AS GarantiaContratoId, dbo.GarantiaContrato.DealReference
FROM         dbo.GarantiaBase LEFT OUTER JOIN
                      ...
                      dbo.Actor AS Administrador ON dbo.GarantiaBase.Administrador = Administrador.ID LEFT OUTER JOIN
                      dbo.GarantiaContrato ON dbo.GarantiaContrato.GarantiaId = dbo.GarantiaBase.ID