...
<% Html.Telerik().ComboBoxFor(m => m.Key)
    ...
    .ClientEvents(c => c.OnChange("onClientChange").OnLoad("onDropdownChange"))
    .Value(Model.Key)
    .Filterable(f=>f.Enabled(true).FilterMode(AutoCompleteFilterMode.Contains).MinimumChars(2))
...
<div class="window">
    ...
    .Title("Formulario de ingreso de Clientes")
    .Visible(false)
    .Draggable(false)
    ...
    {%>
            <table border="0" cellpadding="2" cellspacing="2" style="width:100%;">
                ...
                <tr>
                    <td colspan="2">
                        <%=string.Format("<input id='btnSubmit_{0}' type='button' name='btnSubmit_{1}' class='t-group' style='width:100%;' value='Guardar Cliente Dummy'/>", ViewData.TemplateInfo.GetFullHtmlFieldId(string.Empty), ViewData.TemplateInfo.GetFullHtmlFieldName(string.Empty))%>            
                    </td>
                </tr>
            </table>
        <br />
            ...
            function init_<%=ViewData.TemplateInfo.GetFullHtmlFieldId(string.Empty)%>() {
                ...
                submitButton
                    .bind('click', function (e) {
                        var cName = $(txtElement).val().toString().toUpperCase();
                        $(hiddenElement).val(cName);
                        $(txtElement).val("");
                        windowElement.data('tWindow').close();
                        var combobox = $(dropdown).data('tComboBox');
                        var result = combobox.data.push({ NationalId: "N/A", IsInternal: true, Text: cName, Value: "-1" });
                        combobox.select(result - 1);
                        $(divContainer).html("<i>El Cliente Dummy " + $(hiddenElement).val() + " será generado.</i>");
                        divContainer.show();
                    });
...
<% if(bool.Parse(System.Configuration.ConfigurationManager.AppSettings["EnableCreationOfDummyCustomers"])) {  %>
<br />     
<%} %>
<%=string.Format("<input type='button' id='undo_{0}' name='undo_{1}' class='t-group' value='Ingresar un nuevo cliente' {2} />", ViewData.TemplateInfo.GetFullHtmlFieldId(string.Empty), ViewData.TemplateInfo.GetFullHtmlFieldName(string.Empty), bool.Parse(System.Configuration.ConfigurationManager.AppSettings["EnableCreationOfDummyCustomers"]) ? string.Empty : "style='display:none;'")%>
